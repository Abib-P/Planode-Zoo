...
treatmentRouter.post("/", async function (req, res) {
    const {description, end, start, frequency, nextOccurrence, animal_id} = req.body;

    if (description === undefined || end === undefined || start === undefined || frequency === undefined || nextOccurrence === undefined || animal_id === undefined) {
        res.status(400).end();
        return;
    }
